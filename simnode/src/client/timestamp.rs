...
///
/// This works by either fetching the `slot_number` from the provided header and dividing
...
impl SlotTimestampProvider {
	...
	pub fn new_babe<B, C>(client: Arc<C>, parent: B::Hash) -> Result<Self, Error>
	where
		B: BlockT,
		...
		C::Api: BabeApi<B>,
	{
		...
		let time = Self::with_header(&client, parent, slot_duration, |header| {
	...
	pub fn new_aura<B, C>(client: Arc<C>, parent: B::Hash) -> Result<Self, Error>
	where
		B: BlockT,
		C: AuxStore + HeaderBackend<B> + ProvideRuntimeApi<B> + UsageProvider<B>,
		...
		F: Fn(B::Header) -> Result<u64, Error>,
	{
		let header = client
			.header(parent)?
			.ok_or_else(|| Error::BlockNotFound(format!("{parent}")))?;
